...
from numpy import argsort, ndarray, abs, mean
...
class Cifar100:
    ...
    def __init__(
        ...
        train_data = CIFAR100(root=path, download=True, train=True, transform=transform)
        ...
        if curriculum and not by_batch:
            ...
            index_values = [curriculums[curriculum](image) for image in train_data.data]

            # Sort from middle if needed
            if sort_mean:
                index_values = abs(index_values - mean(index_values))

            # Determine sorted indeces
            sorted_indeces: ndarray = argsort(index_values)
        ...
        test_data = CIFAR100(root=path, download=True, train=False, transform=transform)
        ...
        self._logger.debug(
            f"DATASET: {self} | CLASSES: {self.num_classes} | CHANNELS: {self.channels_in} | DIM: {self.dim}"
        )
