...
class Job:
    ...
    def __init__(self):
        ...
        self._output_dir: str = (
            f"{ARGS.output_path}/{ARGS.model}/{ARGS.dataset}/{ARGS.curriculum}/{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )

        # Fetch dataset
        self._dataset: Cifar10 = get_dataset(
            dataset=ARGS.dataset,
            path=ARGS.dataset_path,
            batch_size=ARGS.batch_size,
            curriculum=ARGS.curriculum,
            by_batch=ARGS.by_batch,
            sort_mean=ARGS.sort_mean,
        )
        self._train, self._test = self._dataset.get_loaders()

        # Fetch model
        self._model: CNN = get_model(
            ARGS.model,
            self._dataset.channels_in,
            self._dataset.num_classes,
            self._dataset.dim,
        )

        # Initialize optimizer
        self._optimizer: SGD = SGD(
        ...
        self._training_report: DataFrame = DataFrame(
            columns=[
                "Epoch",
                "Train Accuracy",
                "Train Loss",
    ...
    def run(self) -> None:
        """Execute job."""
        LOGGER.info(
            f"Exeucting job: MODEL {ARGS.model} | DATASET {ARGS.dataset} | CURRICULUM {ARGS.curriculum} | BY BATCH {ARGS.by_batch} | EPOCHS {ARGS.epochs} | BATCH SIZE {ARGS.batch_size}"
        )

        # Conduct training
        train_acc, train_loss, val_acc, val_loss = self._train_(
            ARGS.curriculum, by_batch=ARGS.by_batch
        )
        ...
        LOGGER.info(
            f"Job complete: MODEL {ARGS.model} | DATASET {ARGS.dataset} | CURRICULUM {ARGS.curriculum} | EPOCHS {ARGS.epochs} | BATCH SIZE {ARGS.batch_size} | TEST ACCURACY: {test_acc}"
        )
    ...
    def _record_results(
        ...
        """
        LOGGER.info(
            f"Recording job results in {path} for {model}, {dataset}, {curriculum}, {by_batch}, {epochs}, {batch_size}"
        )
    ...
    def _train_(
        self, curriculum: str = None, by_batch: bool = False
        ...
        for epoch in range(1, ARGS.epochs + 1):
            ...
            with tqdm(
                ...
                for images, labels in self._train:
                    ...
                    train_correct += accuracy_score(
                        labels.cpu(),
                        argmax(train_predictions, dim=1).cpu().numpy(),
                        normalize=False,
                    )
                ...
                for images, labels in self._test:
                    ...
                    with no_grad():
                        ...
                        # Update correct & total
                        val_correct += accuracy_score(
                            labels.cpu(),
                            argmax(val_predictions, dim=1).cpu().numpy(),
                            normalize=False,
                        )
                ...
                self._training_report.loc[epoch - 1] = [
                    int(epoch),
                    train_accs[-1],
                    train_losses[-1],
                    val_accs[-1],
            ...
            # Log epoch metrics
            LOGGER.info(
                f"Epoch {epoch:>3}/{ARGS.epochs:>3} | Training Accuracy: {train_accs[-1]:>7.4f}% | Training Loss: {train_losses[-1]:>7.4f} | Validation Accuracy: {val_accs[-1]:>7.4f}% | Validation Loss: {val_losses[-1]:>7.4f}"
            )

        # Write distributions to training report & save it
        self._training_report.loc[epoch] = [
            "AVERAGE",
            mean(train_accs),
            mean(train_losses),
            mean(val_accs),
        ...
        self._training_report.loc[epoch + 2] = [
            "MEDIAN",
            median(train_accs),
            median(train_losses),
            median(val_accs),
        ...
        self._training_report.loc[epoch + 3] = [
            "MINIMUM",
            min(train_accs),
            min(train_losses),
            min(val_accs),
        ...
        self._training_report.loc[epoch + 4] = [
            "STD-DEV",
            std(train_accs),
            std(train_losses),
            std(val_accs),
        ...
        with tqdm(
            ...
            for images, labels in self._test:
                ...
                with no_grad():
                    ...
                    # Update correct & total
                    test_correct += accuracy_score(
                        labels.cpu(),
                        argmax(test_predictions, dim=1).cpu().numpy(),
                        normalize=False,
                    )
            ...
            # Calculate accuracy
            self._training_report.loc[ARGS.epochs + 5] = [
                "Test",
                "Accuracy",
                test_acc,
                "Loss",
